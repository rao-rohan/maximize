...
/// 2. **Is this workout unscored, or on a day no plan governs?** Both are real,
///    distinct states — see `Scoring.unscored` and `scheduledSession == nil` — and
///    conflating either with "loading" or "nothing to show" would misrepresent the
///    workout on screen.
///
...
public struct WorkoutVerdict: Hashable, Sendable {
    ...
    /// The scoring verdict, or its honest absence.
    public enum Scoring: Hashable, Sendable {
        /// No automatic score has been recorded for this workout.
        ///
        /// **Not a placeholder for `.scored`.** With MAX-015/MAX-023 still landing,
        /// this is the state of every workout in the app today — the seconds-to-minutes
        /// gap between a run finishing and scoring completing, or, for a workout the
        /// scorer can never reach (no API key stored, no network), an indefinite one.
