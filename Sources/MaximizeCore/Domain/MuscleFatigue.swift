...
public struct MuscleFatigueModel: Hashable, Sendable {
    ...
    ///
    /// ## The seam a strain weighting swaps into
    ///
    /// One case today, and it is written as an enum rather than inlined because
    /// **strain is a better weight than duration**: it is what a session *cost*, where
    /// duration is only how long it took. MAX-179 shipped weighted by duration
    /// deliberately, so it did not depend on work running beside it.
    ///
    /// MAX-176 has since landed `DerivedMetrics.strain` — a nullable `WorkoutStrain`,
    /// Edwards' zone-weighted minutes over the stored heart-rate curve, computed once at
    /// ingestion (D2). Adopting it here is three edits and a decision:
    ///
    /// 1. `MuscleFatigueSession` gains an optional `strainPoints` beside its duration —
    ///    read from the stored record, never recomputed (D2).
    /// 2. A `.strain` case joins this enum and
    ///    `MuscleFatigueCalculator.weight(for:model:)` gains one branch.
    /// 3. **The decision**: `WorkoutStrain.points` is *unbounded above*, and a weight
    ///    here is `0...1`. So a strain weighting needs its own full-session reference —
    ///    the counterpart of `fullSessionSeconds`, saturating the same way — and picking
    ///    that number is a product judgement, not a refactor.
    ///
    /// **Duration does not become dead code when that happens.** A lift whose strap
    /// dropped has no heart-rate curve and therefore no strain — A18, and
    /// `DerivedMetrics` enforces it — and MAX-175's rule says such a session must not be
    /// judged on a fabricated one. So `.duration` stays as the honest fallback, and the
    /// choice between them stays the caller's.
