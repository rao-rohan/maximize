...
extension ScoreCalendarDayState {
    ...
    /// `.scheduledRest`, `.convertedRest` and `.unplanned` all sit on the same neutral
    /// fill (`ScoreBandColors.swift`: a day with no verdict is not a fourth band), which
    /// is fine for four states told apart by their glyphs — but `.forthcoming` would
    /// have been a fifth neutral cell carrying a session glyph, which is very nearly
    /// `.awaitingScore` at a glance and means the opposite thing.
    ///
