...
| MAX-199 | **Haptics, and a way to Settings from a keyless thread** — §6.6, §5.3. `.sensoryFeedback` on send, reply-complete, failure and proposal-arrival; Settings route on the chat sheet so "add a key in Settings" is one tap | 184 | Haiku 🔄 — built, PR #192 open, awaiting review. **No Swift toolchain in this container**, so nothing here was compiled or run (R1); haptics cannot be felt in CI or a simulator, so device verification is essential. See the MAX-199 section below |
