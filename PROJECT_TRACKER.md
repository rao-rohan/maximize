...
| P3 | `Plan` records **no durations at all**, so the "too short to classify" floor can only be distance-based | MAX-013 | A mis-started treadmill run with HR but no distance is not caught and reaches the scorer. Wants `minimumSessionDuration`. **MAX-131 closes this** — a lifting session is prescribed in minutes, so the plan has to learn durations either way |
...
| MAX-126 | A no-verdict state, so the app stops promising a score | MAX-111 | **Opus** ✅ |
| MAX-127 | Widen the surface ramp; re-tune the chart palette | MAX-085 | Sonnet ✅ |
...
roll-up — MAX-134/MAX-135's, and it needs the lifting plan model first.

**No stored score is touched** (D8): a lift ingested before MAX-111 still shows the band it
was given, and A21/MAX-143 remains the owner's call. The detail header drops the spinner and
...
MAX-135 redesigns a ~42pt cell whose contrast budget MAX-084 and MAX-087 already spent.
...
| ID | Ticket | Depends on | Tier |
|---|---|---|---|
| MAX-128 | `Discipline`, and `.lift` on both classification enums | — | **Opus** |
| MAX-129 | The per-discipline prescription; the no-op decode test | 110 | **Opus** |
| MAX-130 | Discipline-gated derived metrics; stop fabricating cadence | 110 | **Opus** |
| MAX-131 | Rubric vocabulary for lifts — **closes gap P3** | 110 | **Opus** |
| MAX-132 | Seed bands for lift days; the `easy.wellOverCap` shadow | 113 | Sonnet |
| MAX-133 | Match a workout to its own discipline's ask | 111, 113 | **Opus** |
| MAX-134 | Obligations, not days: tallies, streak, rest-day budget | 111, 115 | **Opus** |
| MAX-135 | The calendar's mixed day | 116, **105** | **Opus** |
| MAX-136 | Context and fact sheet learn discipline | 111, 112 | **Opus** 🔒 |
| MAX-137 | Plan authoring for two slots | 111 | Sonnet |
| MAX-138 | The plan screen shows both | 111 | Sonnet |
| MAX-139 | Workout detail for a lift | 112, 115 | Sonnet |
| MAX-140 | Trend tiles, honestly ("days run", the effective denominator) | 116 | Sonnet |
| MAX-141 | `PlanProposal` covers lift days | 111, **099** | Sonnet 🔒 |
| MAX-142 | `TrainingContext` is per-session, not per-run | 111, **095** | **Opus** 🔒 |
| MAX-143 | **Decide what to do with lifts already scored as runs** | 110 | Owner / overseer |
...
2. **MAX-095's brief must carry LIFTING-SPEC §10.2 before it is written.** Its specified
   ...
   with the plan block carrying the lift week. Briefing it now costs a paragraph; MAX-142
   exists only if 095 lands unbriefed, and it would be a rewrite of an Opus 🔒 ticket.
3. **MAX-128 adds `.lift` to `RestDayBudgeting.costTier` without reordering the existing
